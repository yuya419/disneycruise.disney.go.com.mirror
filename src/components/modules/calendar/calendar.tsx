...
export default function BookingCalendar({ onDateClick }: { onDateClick: () => void }) {
    const now = new Date();
    ...
    // ダミーデータ
    const dummyData: Record<number, Record<number, { date: string; avail: boolean; price: string }[]>> = {
        [2025 as number]: {
            4: [
    ...
    // 取得する月の範囲を決定
    const futureMonths = Object.keys(dummyData[year] || {}).map((monthKey) => new Date(year, parseInt(monthKey) - 1, 1)).filter((date) => date >= now)
    ...
    // 前後の月を移動するボタン
    const CustomNavigation = ({ month, setMonth }: { month: Date; setMonth: (date: Date) => void }) => {
        ...
        return (
            <div className="parts__navigation">
                <button
                    ...
                    aria-label="前の月"
                    disabled={previousMonth < new Date(now.getFullYear(), now.getMonth(), 1)}>
    ...
    const Calendar = () => {
        ...
        const DayFormatter = (day: any) => {
            ...
            return (
                <td className={`rdp-day${today ? " today" : ""}${!avail ? " avail" : ""}${pastDate ? " past" : ""}${isSaturday ? " isBlue" : ""}${isSunday || isHoliday ? " isRed" : ""}`}>
                    <button
                        type="button"
