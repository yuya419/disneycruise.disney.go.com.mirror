...
export const Slider = (props: SliderProps) => {
  ...
  if (slides.length === 1) {
    return (
      <div className="m-slider" data-type={type}>
        <div className="m-slider-item" role="listitem">
          {slides[0].dom}
        </div>
      </div>
    );
  ...
  const autoPlay = autoplay
    ? useRef(Autoplay({ delay: 4000, stopOnInteraction: false }))
    : undefined;
  ...
  const {
    ...
    onPrevButtonClick,
    onNextButtonClick,
  } = usePrevNextButtons(emblaApi, autoPlay);

  // sliderを生成
  const SliderItems = slides.map((slide, index) => (
    <div className="m-slider-item" key={index} role="listitem">
      {slide.dom}
    </div>
  ));
