      ...
      {Object.entries(queryUnits)
        ...
        .map(([key, unit]) => {
          ...
          if (key === "stay" && typeof value === "object") {
            ...
            const isPeopleQuery = (
              val: any,
            ): val is { adult: number; child: number } =>
              typeof val === "object" && "adult" in val && "child" in val;

            const adultText =
              isPeopleQuery(value) && value.adult ? `大人${value.adult}名` : "";
            const childText =
              isPeopleQuery(value) && value.child ? `子供${value.child}名` : "";
            displayValue = [adultText, childText].filter(Boolean).join(" / ");
          } else {
            displayValue = unit.includes("　")
              ? unit.replace("　", ` ${value}`)
              : `${value}${unit}`;
          }
