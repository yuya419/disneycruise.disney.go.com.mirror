    ...
    return (
        <form className="form">
            ...
            <Block id="input02" title="代表者様情報の入力">
                ...
                <Input type="address" title="代表者住所" name="address" required={true} />
            </Block>
            <Block id="input03" title="お客様情報の入力">
                <div className="form-block-guest">
                    <div className="text">
                        <p>参加する方全員、<br className="nopc"/>下記項目を全てご記入ください。<br />
                            4名以上の場合は追加ボタンを押して記入欄を追加してください。</p>
                    </div>
                        ...
                        {guests.map((guest) => (
                            <dl className="guest-block" key={guest.id}>
                                ...
                                <dd className="guest-block-content">
                                    <Input type="nameEn" title="お名前（姓・名）" name={`nameEn${guest.id}`} required={true} />
                                    <Input type="birthDate" title="生年月日" name={`birthDate${guest.id}`} required={true} />
                                    <Input type="sex" title="性別" name={`sex${guest.id}`} required={true} />
                                    <Input type="nationality" title="国籍" name={`nationality${guest.id}`} required={true} />
                                </dd>
                            </dl>
                        ))}
                    </div>
                    <div className="guest-button">
                        <button type="button" className="guest-button-el" onClick={addGuest}>
