...
export default function Adults() {
    ...
    const generateElements = (array: Record<string, { name: string, en: string, desp: string }>) => {
            ...
            return (
                <div key={key} className={`outline-item ${key == "01" ? "isActive" : "isNext"}`} data-num={key}>
                    ...
                    <div className="outline-image">
                        <Image src={getImagePath(`top/adults/img${key}.jpg`)} alt={array[key].name + 'のイメージ写真'} width={580} height={387} priority />
                    </div>
                </div>
            )
        })
        ...
        const bg = Object.keys(array).map((key) => {
            return (
                <div key={key} id={"outline" + key} className={`${key == "01" ? "relaxation-bg-item isActive" : "relaxation-bg-item"}`} data-num={key}>
    ...
    const Relaxation = () => {
        const [emblaRef, emblaApi] = useEmblaCarousel({ loop: false }, [ClassNames()]);
        const { outline, bg } = useMemo(() => generateElements(topRelaxations), [topRelaxations]);
        ...
        useEffect(() => {
            ...
            // オフセットの更新
            const updateOffsets = () => {
                const bgItem = container.querySelector(".relaxation-bg-item") as HTMLElement;
                const offsets = () => {
