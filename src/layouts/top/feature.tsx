...
import { Feature as Archive } from "@/layouts/post/article";
import Button from "@/components/modules/buttons/button";
import { GSAPMaskToggle } from "@/components/modules/gsap/container";
...
export default function Feature() {
  ...
  const PostThumbnailList = (props: {
    ...
    return <div className="post-thumbnail-list">{thumbnailItems}</div>;
  };
  ...
  useEffect(() => {
    ...
    const updateDividerHeights = () => {
      ...
      if (postItems) {
        const heights = Array.from(postItems).map(
    ...
    if (wrapElement) {
      Array.from(wrapElement.querySelectorAll(".post-item")).forEach((item) =>
  ...
  useEffect(() => {
    ...
    links.forEach((link) =>
      link.addEventListener("mouseenter", (e) => handleMouseEvent(e as Event)),
    );
  }, []);
      ...
      <div className="container">
        ...
        <div className="t-feature__body">
          <div className="feature" data-post="1" ref={hoverGroupRef}>
            ...
            <div className="divider-list">{isDivider}</div>
          </div>
          {posts && (
            <Button
              type="secondary"
