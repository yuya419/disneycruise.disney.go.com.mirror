...
export default function ThemedAreas() {
  ...
  const addRef = () => {
    ...
    const addAreaImageRef = (el: HTMLDivElement | null) => {
      if (
        el &&
        refs.areaImages.current &&
        !refs.areaImages.current.includes(el)
      ) {
  ...
  const generateElements = (
    ...
    const outline = Object.keys(array).map((key) => (
      <div
        key={key}
        className={`outline-item is-outline0${key}`}
        role="listitem"
      >
        ...
        <div className="outline-image">
          <div className="img" ref={addRef().addAreaImageRef}>
            <Image
              src={getImagePath(`top/area/img0${key}.jpg`)}
              alt={`${array[key].name}エリアのイメージイラスト`}
              width={820}
              height={461}
              priority
            />
    ...
    const bg = Object.keys(array).map((key) => (
      <div
        key={key}
        className={`areas-bg-item is-area0${key}`}
        ref={addRef().addBgRef}
      >
        <Image
          src={getImagePath(`top/area/grd/grd-0${key}.png`)}
          alt=""
          width={1300}
          height={830}
          priority
        ...
        />
        <Image
          src={getImagePath(`top/area/img0${key}_sp.jpg`)}
          alt=""
          width={1300}
          height={830}
          priority
          className="image nopc"
        />
        <Image
          src={getImagePath(`top/area/img0${key}.jpg`)}
          alt=""
              ...
              <div className="label">
                ...
                <span className="num" lang="en">
                  ...
                  {Object.keys(topThemedAreas).map((key) => (
                    <span key={key} className={`val is-val0${key}`}>
                      {key}
                    </span>
                  ))}
                </span>
  ...
  const scrollAnimation = () => {
    ...
    const phase03 = () => {
      el.spaces.slice(1, 8).forEach((space) => {
        ScrollTrigger.create({
          ...
          onEnter: () => {
            refs.current.current?.setAttribute(
              "data-current",
              `is-area0${space.dataset.num}`,
            );
          },
          onLeaveBack: () => {
            ...
            refs.current.current?.setAttribute(
              "data-current",
              `is-area${num.toString().padStart(2, "0")}`,
            );
          },
        });
      });
    ...
    const phase04 = () => {
      ...
      const areaImage = (
        ...
        let imgCp =
          areaImage.img.start.cp +
          (areaImage.img.end.cp - areaImage.img.start.cp) * progress;
        let imgY =
          areaImage.img.start.y +
          (areaImage.img.end.y - areaImage.img.start.y) * progress;
        let wrapCp =
          areaImage.wrap.start.cp +
          (areaImage.wrap.end.cp - areaImage.wrap.start.cp) * progress;
        let wrapY =
          areaImage.wrap.start.y +
          (areaImage.wrap.end.y - areaImage.wrap.start.y) * progress;
      ...
      el.spaces.slice(2, 8).forEach((space, index) => {
        ScrollTrigger.create({
          ...
          onUpdate: (self) => {
            ...
            areaImage(
              el.areaImages[index].querySelector("img") as HTMLImageElement,
              el.areaImages[index + 1],
              progress,
            );
          },
        });
  ...
  return (
    <GSAPToggleContainer
      tag="section"
      className="t-themed-areas"
      toggle={{
        logo: false,
        color: "blue",
        movie: { video: "water", state: false },
      }}
    >
      <div className="container" ref={refs.container}>
        <div className="t-themed-areas__head" ref={refs.areasHead}>
          ...
          <hgroup className="headline">
            <p lang="en">
              <span className="num">7</span>Themed Areas
            </p>
            <h2>7つのテーマ別エリア</h2>
          </hgroup>
          <span className="bar is-right"></span>
